...
func TestPurgeCommandMarksKafkaPublicationWithoutClaimingPhysicalDeletion(t *testing.T) {
	api, sessionCookie, csrfCookie, chatID := newChatFixture(t, "purge-kafka@example.com")
	runID := admitRunForLeaseTest(t, api, sessionCookie, csrfCookie, chatID, "0190cdd2-5f2d-7ad8-b3f5-1b588788d444")
	ctx := context.Background()
	var traceID agentobs.TraceID
	if err := api.db.Pool().QueryRow(ctx, `select trace_id from agent_trace_refs where run_id = $1`, runID).Scan(&traceID); err != nil {
		t.Fatal(err)
	...
	if err := store.MarkPurgePublished(ctx, claimed); err != nil {
		t.Fatal(err)
	}
	var state string
	if err := api.db.Pool().QueryRow(ctx, `select delivery_state from agentobs_outbox_commands where trace_id = $1`, traceID).Scan(&state); err != nil {
	...
	if state != "published" || objects.Len() != 0 {
		t.Fatalf("purge state=%q producer_objects=%d", state, objects.Len())
	}
	if _, ok, err := store.ClaimPurgeBatch(ctx); err != nil || ok {
		t.Fatalf("published purge was claimable ok=%t err=%v", ok, err)
	}
}
