...
- **Live end-to-end validation (2026-08-01, post-acceptance):** Brought up the full stack (control-plane, worker, collector, fetcher, Prometheus, Grafana) and drove a real user session through the API with live credentials — register, create a Notebook, trigger the leader→research delegation with a real web search (Brave), import discovered candidates as Sources, wait for real ingestion/indexing, and ask grounded follow-up questions that produced real citations. This is real production traffic, not a fixture. It found and fixed three genuine bugs in the `source_processing` metrics path (see the `fix:` commit immediately following the original acceptance commits): `sourcejobs.Queue.CompleteEvidence` — the actual production success path — never called any metrics method; the `task_variant` allowlist mirrored the wrong DB column's domain; and `TaskMetricsRecorder.variantAllowlist` never dispatched `source_processing` to any allowlist at all. After the fixes, a repeated live cycle confirmed `nano_task_terminal_total{task_kind="source_processing",outcome="completed",task_variant="url"}` increments correctly. This run also empirically confirmed Known Deviation #1 (legacy_role completions are not counted: one of three `agent_run` completions — the one that took the research-delegation path — was silently uncounted) and separately observed `nano_retrieval_degraded_total{degradation="reranker_unavailable"}` fire correctly on a real transient reranker outage while the search still completed, which is exactly the failure mode that metric exists to catch. All leak gauges read exactly zero at rest during this run.
