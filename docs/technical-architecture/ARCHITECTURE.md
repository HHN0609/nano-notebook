...
Sprint 9 moves production model instructions into an immutable Git-owned Prompt Catalog. Markdown Prompt Versions and typed model-facing contracts are embedded and registered durably by exact identity and hash. Leader admission pins one immutable Agent Configuration Set; a Research child carries the same Set identity but resolves a Role-specific Profile for model, Prompt, Provider, local budget, and executor compatibility. Infrastructure continuation never resolves a mutable `latest` Prompt or configuration.

Sprint 9 splits the prior combined Leader executor into an Agent Worker execution host, application-registered Role Registry, Leader and Research Role Executors, Prompt Runtime, and bounded Delegation Kernel. The Kernel is the sole generic authority for child creation, parent waiting, cancellation propagation, terminal handoff, parent wake-up, and consumption. A generic Agent Delegation owns the parent-child relationship and terminal lifecycle; Research separately owns its Discovery Session outcome. Only `leader -> research`, depth one, one active child, and no child delegation are registered.

Leader routing is a typed model intent decision followed by deterministic Delegation Policy. The model cannot grant Web access, Role relationships, or Member authority. Router and Research Planner use required function-call contracts and fail closed. The current user turn must explicitly request new Source Discovery; bounded recent conversation may resolve references but cannot create a sticky Research mode.

Role execution ends through a typed Attempt Disposition. Classified transient errors deliberately requeue the same Job with bounded `available_at` backoff under its pinned Profile and root deadline; Lease expiry remains the recovery fallback when the disposition cannot commit. This Sprint 9 policy supersedes the earlier blanket rule that every terminal Provider timeout or unavailable result immediately fails an Agent Run, while retaining bounded gateway retries and Role Attempt limits.

Research checkpoints its accepted query plan and each accepted sequential Web Search result. Recovery skips accepted ordinals and repeats only missing or commit-uncertain work. External calls remain bounded at-least-once; accepted product state and publication remain fenced and idempotent. The child shares the Leader's absolute root deadline, and waiting never pauses or extends it.

No generic workflow SDK, arbitrary DAG, deterministic code replay, exactly-once promise, general multi-Agent runtime, dynamic Role, Agent plugin, or Agent Sandbox enters the first release. An external MQ is metrics-triggered evolution rather than scheduled scope. If PostgreSQL dispatch becomes a measured bottleneck in the intended AWS environment, SQS plus a transactional outbox is the preferred direction while PostgreSQL retains authoritative Job and Run state.
...
Source opening uses one server-declared action across the Sources list and Chat citations. `external` is a canonical web or YouTube URL opened in a new browser tab; `inline_original` is an authorized, integrity-checked immutable original rendered only in the left Sources panel; `none` is non-interactive. The browser does not infer support from an extension, fetch processed Evidence for presentation, show Citation excerpts, or render normalized/derived artifacts. The app provides no download action, although native browser controls may still permit saving inline content. This Sprint 8 contract supersedes the Sprint 6 processed Source Viewer interaction without changing ingestion, Retrieval, grounding, Citation persistence, or deletion semantics.
