...
Every production executable Agent tool will be discovered and invoked through one internal Nano Tools MCP Server. Sprint 10 migrates `calculate`, `current_time`, `search_evidence`, and `web_search`, and generates definition-specific `delegate.*` tools from immutable Agent Definitions. Ordinary tools return business results synchronously; an accepted delegation call returns an internal scheduling receipt and causes Nano's Controller to suspend the parent until the durable child result exists.
...
Tool adapters register scheduling classes that configuration cannot widen. `ordered_sync` tools execute proposal order sequentially and checkpoint each accepted Result. A generated delegation tool is `exclusive_delegation` and must be the proposal's only Action because it suspends the parent. Sprint 10 adds no parallel tool execution.
...
Long-running delegation follows ADR 0042: standard MCP discovery and `tools/call` carry the generated capability, while Nano's PostgreSQL runtime owns waiting, completion, cancellation, and recovery. The first transport is in-process, but the generated capability still traverses the same MCP discovery, invocation, metadata, and error contracts so the old direct Action path does not survive beside it.
...
Implementation note (2026-07-28): the synchronous tool plane is active for `calculate`, `current_time`, `search_evidence`, and `web_search`. Generated `delegate.*` tools use the same official SDK and standard `tools/call`; the scheduling receipt is internal Controller data and is never represented as an MCP Task or model-visible completion.
